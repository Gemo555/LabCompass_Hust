...
项目只处理公开网页，不登录任何网站，不访问私人账号，不绕过访问限制。默认只从少量学院官网师资列表页中抽取样例导师主页，先保证流程跑通。
...
```bash
python src/scrape_teachers.py --limit 10 --delay 3
```
...
2. 运行 `python src/scrape_teachers.py --limit 10 --delay 3`。
3. 检查 `output/teachers.csv` 中 `抓取状态`、`备注`、`来源URL`。
4. 对字段为空或标注“未找到”的老师，打开 `data/raw_text/` 或原网页人工核验。
...
- 默认访问间隔为 2 秒，可以用 `--delay` 调大。
- 请求失败、结构复杂或字段无法确定时，脚本保存原始 HTML/文本并在表格中留空或标注“未找到”。
- 不伪造导师数据，不尝试登录，不使用私人账号。

## 后续 TODO

- [ ] 扩充 `config/seed_pages.json`，覆盖学院其他师资分组。
- [ ] 增加人工修订表与自动抓取表的合并脚本。
- [ ] 为不同研究方向建立更细的本科生任务类型标签。
- [ ] 增加重复老师合并和旧页面识别。
